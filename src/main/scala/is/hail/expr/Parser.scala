...
object Parser extends JavaTokenParsers {
  ...
  def matrix_ir_1: Parser[ir.MatrixIR] =
    "MatrixFilterCols" ~> matrix_ir ~ ir_value_expr() ^^ { case child ~ pred => ir.MatrixFilterCols(child, pred) } |
      ...
      "MatrixMapRows" ~> string_literals_opt ~ string_literals_opt ~ boolean_literal ~ matrix_ir ~ ir_value_expr() ^^ { case newKey ~ newPartitionKey ~ shuffle ~ child ~ newCol =>
        val newKPK = ((newKey, newPartitionKey): @unchecked) match {
          case (Some(k), Some(pk)) => Some((k, pk))
          case (None, None) => None
        }
        ir.MatrixMapRows(child, newCol, newKPK, shuffle)
      } |
